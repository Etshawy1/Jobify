...
export default {
  data: () => ({
    ...
    snackbar: false,
    headers: [
      { text: "Category Name", sortable: false, value: "name" },
  ...
  methods: {
    ...
    async deleteItemConfirm() {
      try {
        ...
        await this.$store.dispatch("adminDeleteCategory", {
        ...
        this.closeDelete();
        this.snackbar = true;
        this.snackbarText = "failed to delete category, try again later";
      }
    },
    ...
    async save() {
      if (this.editedIndex > -1) {
        try {
          this.loadingState = true;
          await this.$store.dispatch("adminUpdateCategory", {
            userToken: localStorage.getItem("userToken"),
            ...this.editedItem,
          });
          ...
          this.close();
          this.snackbar = true;
          this.snackbarText = "failed to edit category, try again later";
        }
      } else {
        try {
          this.loadingState = true;
          await this.$store.dispatch("adminAddCategory", {
            userToken: localStorage.getItem("userToken"),
            ...this.editedItem,
          });
          await this.getDataFromApi();
          ...
          this.close();
          this.snackbar = true;
          this.snackbarText = "failed to add category, try again later";
        }
      }
    },
