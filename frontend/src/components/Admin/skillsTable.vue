...
export default {
  data: () => ({
    ...
    snackbar: false,
    headers: [
      { text: "Skill Name", sortable: false, value: "name" },
  ...
  methods: {
    ...
    async deleteItemConfirm() {
      try {
        ...
        await this.$store.dispatch("adminDeleteSkill", {
        ...
        this.closeDelete();
        this.snackbar = true;
        this.snackbarText = "failed to delete skill, try again later";
      }
    },
    ...
    async save() {
      if (this.editedIndex > -1) {
        try {
          this.loadingState = true;
          await this.$store.dispatch("adminUpdateSkill", {
            userToken: localStorage.getItem("userToken"),
            ...this.editedItem,
          });
          ...
          this.close();
          this.snackbar = true;
          this.snackbarText = "failed to edit skill, try again later";
        }
      } else {
        try {
          this.loadingState = true;
          await this.$store.dispatch("adminAddSkill", {
            userToken: localStorage.getItem("userToken"),
            ...this.editedItem,
          });
          await this.getDataFromApi();
          ...
          this.close();
          this.snackbar = true;
          this.snackbarText = "failed to add skill, try again later";
        }
      }
    },
